<template>
    <div class="container">
        <div class="page-header">
            <h1 class="page-title" style="white-space:pre;">Rekap Berdasarkan Guru </h1>
        ...
        <div class="row row-cards">
            <div class="col-8">
                <div class="card">
                    <div class="card-header">
                        <h5 style="margin-bottom: 0 !important;">Persentase Kehadiran Siswa Berdasarkan Guru</h5>
                    ...
                    <div v-else class="card-body" style="padding:0 !important;">
                        <div class="input-group">
                            <input type="text" class="form-control" v-model="search" v-on:keyup="searchEnter" placeholder="Cari Nama Guru" style="border-radius: 0 !important;border: none;">
                        ...
                        <div class="rekaplist">
                            <div class="rekaplist-item" v-for="(data,key) in tableData" :key="key">
                                <p style="margin-bottom:0!important;margin-top: 6px;text-transform:capitalize;width: 220px;">{{data.nama.substring(0,22).toLowerCase()}} </p>
            ...
            <div class="col-4">
                <div class="card">
                    <div class="card-header">
                        <h5 style="margin-bottom: 0 !important;">Rincian Kehadiran Siswa Berdasarkan Guru</h5>
                    </div>
                    <div class="card-body" style="padding:0 !important;">
                    <div v-if="rinciaData == ''" class="rincianDataKosong">
                            <h5 class="text-muted"> Pilih Rincian Guru</h5>
                    </div>
                    <div v-else class="rincianData">
                        <div v-if="rinciaData.foto" class="avatar d-block rinciFoto" v-bind:style="{ 'background-image':'url(./storage/guru/'+ rinciaData.foto +')' }"></div>
...
export default {
    ...
    methods: {
        /** Method **/
        fetchData() {
            let dataFetchUrl = `${this.url}/api/rekap/guru?page=${this.currentPage}&tanggal=${this.bulan}&search=${this.search}&order=${this.sortedColumn}&by=${this.order}&limit=${this.perPage}`
        ...
        rincian(data){
          ...
          for (var key in this.guruData) {
              if (this.guruData.hasOwnProperty(key)) {
                if(data.user_id == this.guruData[key].id)
                  {
                    this.rinciaData.foto = this.guruData[key].foto;  
                    this.rinciaData.jenis = this.guruData[key].jenis;  
                  }
              }
          }          
        },
