<template>
	<modal name="siswa-upload" draggable=".card-header" :width="400" :height="432" @before-open="beforeOpen" @before-close="beforeClose">
	<div class="card">
	    <div class="card-header card-header-modal">
	     <h5 style="position: absolute; margin-top: 15px;">Foto : <b>{{nama}}</b></h5>
	     <div class="card-options">
	     	<button class="close" @click="$modal.hide('siswa-upload')">
...
export default {
  ...
  computed:{
  	 initialImage() {
      return window.location.origin+'/storage/siswa/'+this.foto
  	 }
  },
  methods: {
  	cleanString(string) {
    ...
    this.croppa.generateBlob((blob) => {    	
    	const url = window.location.origin+'/api/upload/siswa';
    	const config = { headers: { 'Content-Type': 'multipart/form-data' } };
        var fdata = new FormData()
        fdata.append('files', blob, this.cleanString(this.nama)+"_"+this.id+".jpg")
	      ...
	      .then(response => {
	        ...
	        { 
	          ...
	          this.$notify({
	            ...
	            type: 'success',
	            text: 'Foto Siswa '+this.nama+' Berhasil Di Update'
	          });
	          this.$modal.hide('siswa-upload')
	        }
	      })
	      .catch(error => {
	      	 this.disable = false;
