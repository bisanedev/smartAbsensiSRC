<template>
<modal name="absen-edit" draggable=".card-header" :width="400" :height="533.4" @before-open="beforeOpen" @before-close="beforeClose">
  <div class="card">
    <div class="card-header card-header-modal">
     <h5 style="position: absolute; margin-top: 15px;">Absensi {{mapel_guru}}</h5>
      <div class="card-options">
       <button class="close" @click="$modal.hide('absen-edit')">
    ...
    <div class="form-row">
      ...
      <div class="col-md-6" style="margin-bottom: 38px;">
        ...
        <v-select :options="kelasData" label="nama" :reduce="nama => nama.id" v-model="kelas_id" placeholder="Pilih Kelas" @input="onKelas($event)" style="position: fixed;width: 13.8%;z-index: 999;" disabled="disabled">
        </v-select>         
      </div>
      ...
      <div class="col-md-6">
        ...
        <date-picker v-model="masuk" placeholder="Masuk Kelas" format="HH:mm" value-type="format" type="time"></date-picker>        
      </div>
      <div class="col-md-6">
      ...
      <div class="float-right" style="margin-top:20px;">
        <button class="btn btn-primary" :disabled="disable" @click="updateuser()">Update Data</button>
      </div> 
    </div>
  </div>   
  </modal>
  ...
  methods: {
    ...
    updateuser:function(event){
        if(this.kelas_id == null || this.start == null || this.end == null || this.masuk == null || this.keluar == null){
          this.$notify({
              group: 'notifikasi',                  
              title: 'Update Absensi Gagal',
              type: 'error',
              text: 'Periksa Kembali Update Absensi'
            });
          return
        }
        this.disable = true;      
        axios.patch(window.location.origin+'/api/absensi/'+this.id, {
        tanggal:moment(this.tanggal+" "+this.masuk).format('YYYY-MM-DDTHH:mm'),keluar:moment(this.tanggal+" "+this.keluar).format('YYYY-MM-DDTHH:mm'),user_id:Number(this.user_id),jam:Number(this.jam),kelas:this.kelas,mapel:this.mapel,mapelguru:this.mapel_guru,mapel_start:moment(this.tanggal+" "+this.start).format('YYYY-MM-DDTHH:mm'),mapel_end:moment(this.tanggal+" "+this.end).format('YYYY-MM-DDTHH:mm'),piketguru:this.guru_piket,piketid:Number(this.piket_id)
        })
        .then(response => {  
          this.$parent.fetchData();      
          if(response.data.status == "success")
          {             
            this.$modal.hide('absen-edit')
            this.$notify({
              group: 'notifikasi',                  
              title: 'Input Absensi Sukses',
              type: 'success',
              text: 'Data Absensi '+this.mapel_guru+' Berhasil Di Update'
            });
          }
        })
        .catch(error => {
    ...
    beforeOpen (event) {            
      this.kelas_id = event.params.wow.kelas_id;
      this.tanggal = moment(event.params.wow.tanggal).format('YYYY-MM-DD');
      this.fetchKelas();      
      this.fetchJadwal();      
      this.user_id = event.params.wow.user_id;
      this.jam = event.params.wow.jam;      
      this.mapel = event.params.wow.mapel;
      this.mapel_guru = event.params.wow.mapel_guru;
      this.guru_piket = event.params.wow.piket_guru;
      this.piket_id = event.params.wow.piket_id;
      this.masuk = moment(event.params.wow.tanggal).format('HH:mm');
      this.keluar = moment(event.params.wow.keluar).format('HH:mm');
      this.start = moment(event.params.wow.mapel_start).format('HH:mm');
      this.end = moment(event.params.wow.mapel_end).format('HH:mm');      
      this.fetchGuru();
    ...
    beforeClose (event) {
      ...
      this.guruData=[];
      this.kelasData=[];
      this.tanggal = this.keluar = this.kelas_id = this.start = this.end = this.masuk = this.user_id = null;
      this.jam = this.kelas = this.mapel = this.mapel_guru = this.maxpel = this.guru_piket = "";
    }
   //end method
  }
}
