                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-options">
                            ...
                            <input type="text" class="form-control" v-model="search" v-on:keyup="searchEnter" placeholder="Search Tahun Pelajaran">
                            ...
                            <table class="table table-striped table-vcenter text-nowrap card-table">
                                <thead class="thead-dark">
                                    <tr>
                                        ...
                                        <th class="text-center" @click="sortByColumn('nama')">
                                        Tahun Pelajaran
                                        ...
                                        <th class="text-center" @click="sortByColumn('waktustart')">
                                        Tanggal Awal Semester
                                        <span v-if="'waktustart' === sortedColumn">
                                        <i v-if="order === 'asc' " class="fe fe-arrow-up"></i>
                                        <i v-else class="fe fe-arrow-down"></i>
                                        </span>
                                        </th>
                                        <th class="text-center" @click="sortByColumn('waktuend')">
                                        Tanggal Akhir Semester
                                        <span v-if="'waktuend' === sortedColumn">
                                        <i v-if="order === 'asc' " class="fe fe-arrow-up"></i>
                                        <i v-else class="fe fe-arrow-down"></i>
                                        </span>
                                        </th>                                        
                                ...
                                <tbody>
                                    ...
                                    <tr v-else v-for="(data, key1) in tableData" :key="data.id">
                                        ...
                                        <td class="text-center">   
                                        {{data.waktustart | formatDate}}
                                        </td>
                                        <td class="text-center"> 
                                        {{data.waktuend | formatDate}}   
                                        </td>
                                        <td class="text-center">
                                            <div class="item-action dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">    
                                                    <button type="button" @click="showEditModal(data)" class="dropdown-item"><i class="fe fe-edit"></i> Edit 
                                                     ...
                                                     <button type="button" @click="showDeleteDialog(data.id,data.nama,data.semester)" class="dropdown-item"><i class="fe fe-trash"></i> Hapus 
                                                    </button>                                           
                                                </div>
                                            </div>
                                        </td>
...
import SemesterAddModal from './SemesterAddModal.vue'
...
export default {
  ...
  methods: {
    fetchData() {
      let dataFetchUrl = `${this.url}/api/semester?page=${this.currentPage}&search=${this.search}&order=${this.sortedColumn}&by=${this.order}&limit=${this.perPage}`
    ...
    /** User ADD **/
    showAddModal(){
      this.$modal.show('semester-add');
    },
    /** edit modal **/
    showEditModal(data){      
      this.$modal.show('semester-edit',{wow:data});
    },
    /** pass variable to modal **/ 
    showDeleteDialog (id,nama,semester) {
      this.$modal.show('dialog', {
        title: 'Konfirmasi',
        text: 'Do you really want to delete? Semester : '+nama+" "+semester,
        buttons: [
          {
            title: 'Cancel',            
            handler: () => {
          ...
          },
          {
            ...
            default: true,
            handler: () => {                          
              axios.delete(this.url+'/api/semester/'+id)
                .then(response => {                
